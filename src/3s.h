...
#define NGENE                 3  /* required by ReadSeq, but not really in use */
...
typedef struct {
    ...
    int usedata, verbose, aroundMLEM0, nthreads, fixto0[14], fix[12];
    int model_2sp, gIM_2sp, fixto0_2sp[12], fix_2sp[2];
    int nGtree, ngene, seqtype, ns, ls, posG[1+1], lgene[NGENE], *pose, npatt;
    int readpattern, simmodel, runmodels[NEXTMODELS];
    int paraNamesMap[MAXPARAMETERS], paraMap[MAXPARAMETERS], printMap[MAXPARAMETERS];
    double *fpatt, kappa, alpha, rho, rgene[1], pi[4], piG[1][4], initials[NMODELS][MAXPARAMETERS+1];
    double *pDclass, *tau1beta, *bp0124[MAXGTREES+1], *wwprior[MAXGTREES+1];
    BTEntry ** GtreeTab;
    double * space; // free space to avoid many allocs
    int np, ntime, clock, maxnp;
    double *conP;
    double theta_UB, M_UB;
} CommonInfo;
...
typedef struct { /* locus-specific sequence or gene tree information */
    int ndata;
...
enum {M2_1 = 2, M2_2, M2_3, M2_4, M3_1, M3_2, M3_3} /*MODELS2SP*/;
